...
public class DatabaseManager {
    ...
    public List<String> getServerMembers(String serverName) {
        ...
        try (PreparedStatement p = connection.prepareStatement("SELECT username FROM server_members WHERE server_name = ?")) {
    ...
    public String createInvite(String serverName) {
        ...
        try (PreparedStatement u = connection.prepareStatement("INSERT INTO invites(code, server_name) VALUES(?, ?)")) {
            u.setString(1, code); u.setString(2, serverName);
            u.executeUpdate();
            return code;
    ...
    public boolean leaveServer(String username, String serverName) {
        try (PreparedStatement p = connection.prepareStatement("DELETE FROM server_members WHERE username=? AND server_name=?")) {
            p.setString(1, username); p.setString(2, serverName);
    ...
    public List<String> getRelevantHistory(String username) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                history.add("HISTORY:" + rs.getString("context") + "///" + rs.getString("sender") + "///" + rs.getString("content") + "///" + rs.getString("timeStr"));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return history;
    }
