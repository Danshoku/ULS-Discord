...
public class DatabaseManager {
    ...
    public List<String> getRelevantHistory(String username) {
        ...
        // Récupère les messages publics (Serveurs) ET les messages privés qui me concernent
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, "MP:" + username + ":%"); // Cas où je suis le premier
            pstmt.setString(2, "MP:%:" + username);     // Cas où je suis le deuxième
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                // CORRECTION : Utilisation de /// comme séparateur
                history.add("HISTORY:" + rs.getString("context") + "///" + rs.getString("sender") + "///" + rs.getString("content"));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return history;
    }
